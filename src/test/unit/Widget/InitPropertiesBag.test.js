...
test('fromQueryString parses a valid query string', done => {
  ...
  const props = InitPropertiesBag.fromQueryString(`${InitPropertiesBag.PROP_NAMES.dpWidgetId}=${encodeURIComponent('something&something')}&dp.otherNotYetUsed=entirely_other`);
...
test('fromWindow favors the hash property over the query string', done => {
  ...
  const windowObject = {
    location : {
      search: `?${InitPropertiesBag.PROP_NAMES.dpWidgetId}=${encodeURIComponent('unexpected.value')}&dp.otherNotYetUsed=entirely_other`,
      hash: `#${InitPropertiesBag.PROP_NAMES.dpWidgetId}=${encodeURIComponent(expectedValue)}&dp.otherNotYetUsed=entirely_other`
    },
...
test('fromWindow uses the query string if hash is missing ', done => {
  ...
  const windowObject = {
    location : {
      hash: `#`,
      search: `?${InitPropertiesBag.PROP_NAMES.dpWidgetId}=${encodeURIComponent(expectedValue)}&dp.otherNotYetUsed=entirely_other`
    },
    document: { readyState : 'notReady' }
  };
  const initProps = InitPropertiesBag.fromWindow(windowObject);
